...
describe("WebhookOutput", () => {
    describe("send()", () => {
        ...
        then response is false with a specific log message`, async () => {
            ...
            // then
            expect(mockedAxios.post).toHaveBeenCalledTimes(4); // because we have 2 channels in the channel.json file and we send 2 messages to each channel (game + contact message)

            expect(logger.error).toHaveBeenCalledWith(
                "Error while sending message to Discord server: [Size Up], channel: [webhook-testing] with status code: 404"
            );
        ...
        then response is false with a specific log message`, async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith(
                "Sending message to Discord webhook server: [Size Up], channel: [webhook-testing] ..."
            );
            expect(logger.info).toHaveBeenCalledWith(
                "Message sent to Discord server: [Size Up], channel: [webhook-testing]"
            );
            expect(logger.info).toHaveBeenCalledWith(
                "Sending message to Discord webhook server: [BarberCrew], channel: [games] ..."
            );
